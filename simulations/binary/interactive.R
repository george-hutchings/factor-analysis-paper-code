### Interactive binary sim - set slurm_id and source()
slurm_id = 0

algo_path = "~/factor-analysis/mcem_fa_algorithm.R"
method_names = c('our method', 'our method long v0', 'nmf K known', 'li 2023 mirt')
...
generate_binary_probs = function(n) lapply(seq(0.25, 0.75, length.out = n), function(x) c(x, 1 - x))

MakeDiscrete = function(Y, probs, dims = seq_along(probs)) {
  for (i in seq_along(dims)) {
    x = cumsum(probs[[i]])
    Y[, dims[i]] = stepfun(x, seq_len(length(x) + 1), right = TRUE)(pnorm(scale(Y[, dims[i]])))
  }
  Y
}
...
Y = MakeDiscrete(Z, generate_binary_probs(D), seq_len(D))
...
if (slurm_id_modn == 1) {
  ...
  K = ncol(lambda)
  dpe_func = function(v0s, ...) list(list(w = NA, lambda = NA, lambda_corr = my_nmf(...)))
